...
public class UserOperationClaimBusinessRules : BaseBusinessRules
{
    ...
    {
        bool doesExist = await _userOperationClaimRepository.AnyAsync(predicate: uoc => uoc.Id == id);
    ...
    {
        bool doesExist =
            await _userOperationClaimRepository.AnyAsync(uoc => uoc.UserId == userId && uoc.OperationClaimId == operationClaimId);
