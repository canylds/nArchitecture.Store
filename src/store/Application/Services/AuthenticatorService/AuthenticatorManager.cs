...
public class AuthenticatorManager : IAuthenticatorService
{
    ...
    {
        EmailAuthenticator? emailAuthenticator = await _emailAuthenticatorRepository.GetAsync(predicate: ea => ea.UserId == user.Id);
        ...
        {
            new(name: user.Email, address: user.Email)
        };
    ...
    {
        OtpAuthenticator? otpAuthenticator = await _otpAuthenticatorRepository.GetAsync(predicate: oa => oa.UserId == user.Id);
