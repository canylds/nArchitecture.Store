...
public class OperationClaimManager : IOperationClaimService
{
    ...
    {
        OperationClaim? operationClaim = await _operationClaimRepository.GetAsync(predicate,
            include,
            withDeleted,
            enableTracking,
            cancellationToken);
    ...
    public async Task<IPaginate<OperationClaim>?> GetListAsync(Expression<Func<OperationClaim, bool>>? predicate = null,
        ...
        CancellationToken cancellationToken = default)
    {
        IPaginate<OperationClaim> operationClaimList = await _operationClaimRepository.GetPagedListAsync(predicate,
            orderBy,
            include,
            index,
            size,
